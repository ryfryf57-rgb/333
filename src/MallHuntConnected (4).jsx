...
const ArDemoScreen = ({ onBack }) => {
  ...
  const startAR = async () => {
    ...
    try {
      ...
      // ВАЖНО: по умолчанию существо скрыто. MindAR иногда рендерит anchor.group
      // как visible до того как получил первый кадр — это приводит к тому что
      // существо "появляется просто так". Управляем видимостью вручную.
      ...
      const handleTap = () => {
        if (!anchor.group.visible) return; // ловим только когда маркер в кадре
